...
export class EthereumProvider extends BaseProvider {
  ...
  constructor(config) {
    ...
    this.on = (...args) => {
      logEthereum(`on: ${JSON.stringify(args)}`);
      if (args[0] == "connect") {
        setTimeout(() => {
  ...
  setAddress(address) {
    logEthereum(`setAddress: ${address}`);

    const lowerAddress = (address || "").toLowerCase();
    this.address = lowerAddress;
    this.ready = !!address;

    logEthereum(`setAddress: ${this.address}`);
    logEthereum(`setAddress: ${this.ready}`);
   ...
   */
  send(payload) {
    logEthereum(`==> send payload ${JSON.stringify(payload)}`);
  ...
  _request(payload, wrapResult = true) {
    ...
    logEthereum(`==> _request payload ${JSON.stringify(payload)}`);
    ...
    return new Promise((resolve, reject) => {
      ...
      switch (payload.method) {
        ...
        default:
          logEthereum(`<== callback delete: ${payload.id}`);
          // call upstream rpc
          ...
          return this.rpc
            .call(payload)
            .then(response => {
              logEthereum(`<== rpc response ${JSON.stringify(response)}`);
  ...
  personal_sign(payload) {
    logEthereum(`personal_sign: ${JSON.stringify(payload)}`);
    const message = payload.params[0];
    const from = payload.params[1];
    const buffer = Utils.messageToBuffer(message);
    if (buffer.length === 0) {
      logEthereum("personal_sign: Buffer length is 0");
      // hex it
      ...
      logEthereum("personal_sign: Buffer length is not 0");
  ...
  eth_signTypedData(payload, version) {
    logEthereum(`signTypedMessage: ${JSON.stringify(payload)}`);
    ...
    switch (method) {
      ...
      case "signTransaction":
        ...
        window.ethereum
          ...
          .catch(err => {
            logEthereum(`signTransaction error: ${err}`);
  ...
  sendResponse(id, result) {
    ...
    if (callback) {
      ...
      logEthereum(`<== callbackId wrapResult: ${JSON.stringify(wrapResult)}`);
      wrapResult ? callback(null, data) : callback(null, result);

      logEthereum(`<== callback delete: ${id}`);
      this.callbacks.delete(id);
    } else {
      logEthereum(`<== callbackId: ${id} not found`);
      ...
      for (var i = 0; i < window.frames.length; i++) {
        ...
        try {
          ...
          logEthereum(`send response to frame error: ${error}`);
        }
      }
    }
  }
