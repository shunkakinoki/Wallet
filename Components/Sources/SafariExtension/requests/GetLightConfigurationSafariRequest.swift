...
final class GetLightConfigurationSafariRequest: SafariExtensionRequest {
  ...
  var response: String? {
    do {
      ...
      let serializedAccounts = accounts.reduce(
        into: [String: LightConfigurationAccountParameters]()
      ) {
        $0[$1.address.eip55Description] = LightConfigurationAccountParameters(
          name: $1.name.addingPercentEncoding(withAllowedCharacters: .alphanumerics) ?? "",
          icon: $1.color.rawValue)
      }
