...
public final class PostSignatureSafariRequest: SafariExtensionRequest {
    ...
    public init(parameters: Any?, getWallet: GetWallet = GetWalletImp(), signatureType: SignatureType, signature: PersonalMessageRequestSignature = PersonalMessageRequestSignature()) {
    ...
    public var response: String? {
        do {
            ...
            let messageSigned = try signature.signed(with: personalMessageRequest, wallet: getWallet.get(from: from.eip55Description))
...
extension PostSignatureSafariRequest {
    func parametersToStruct(_ parameters: Any?) throws -> PostSignMessageRequestModel {
        ...
        let signatureParameters = try JSONDecoder().decode(PostSignMessageRequestModel.self, from: decodedParameters.toData())
