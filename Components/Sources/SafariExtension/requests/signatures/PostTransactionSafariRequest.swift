...
final class PostTransactionSafariRequest: SafariExtensionRequest {
    ...
    public init(parameters: Any?, getWallet: GetWallet = GetWalletImp(), signature: TransactionRequestSignature = TransactionRequestSignature()) {
    ...
    var response: String? {
        do {
            ...
            let transactionSigned = try signature.signed(with: transactionRequest, wallet: getWallet.get(from: from.eip55Description))
...
extension PostTransactionSafariRequest {
    func parametersToStruct(_ parameters: Any?) throws -> PostTransactionRequestModel {
        ...
        let transactionParameters = try JSONDecoder().decode(PostTransactionRequestModel.self, from: decodedParameters.toData())
