...
public final class SecureStorage: SecureStoraging {
    ...
    public func encrypt(_ address: String, privateKey: Data) throws -> CFData {
        ...
        guard let publicKey = keychain.SecKeyCopyPublicKey(key),
              let ciphertext = keychain.SecKeyCreateEncryptedData(publicKey, .eciesEncryptionCofactorVariableIVX963SHA256AESGCM, privateKey as CFData, error: &error) else {
    ...
    public func decrypt(_ address: String, cipherText: Data) throws -> Data {
        ...
        var error: Unmanaged<CFError>?
        guard let plainTextData = keychain.SecKeyCreateDecryptedData(raw as! SecKey, .eciesEncryptionCofactorVariableIVX963SHA256AESGCM, cipherText as CFData, error: &error) as Data? else {
