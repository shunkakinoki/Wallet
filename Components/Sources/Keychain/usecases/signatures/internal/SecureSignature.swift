...
public final class SecureSignature: SecureSigning {
    ...
    public func signature(hash _hash: Array<UInt8>, privateKey: Data) throws -> (v: UInt, r: ByteArray, s: ByteArray) {
        let signature = try sign(hash: _hash, privateKey: privateKey)
        return (v: UInt(signature.recId), r: Array(signature.sign[0..<32]), s: Array(signature.sign[32..<64]))
    }

    internal func signHash(hash _hash: Array<UInt8>, privateKey: Data) throws -> ByteArray {
        let signature = try sign(hash: _hash, privateKey: privateKey)
...
extension SecureSignature {
    private func sign(hash _hash: Array<UInt8>, privateKey: Data) throws -> (recId: Int32, sign: ByteArray) {
        var hash = _hash
