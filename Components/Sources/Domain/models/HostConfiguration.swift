...
public struct HostConfigurationImp: HostConfiguration {
  ...
  public init(
    hostsDirectory: WriterDirectory = UserDefaultsWalletWriterDirectory(),
  ...
  public func fetchHostsConfiguration() -> [HostConfigurationModel.HostConfigurationParameters]? {
    do {
      if let selectedAccount: EthereumWallet = try ethereumAccount.fetchSelectedWallet() {
        let hostConfigurations: [HostConfigurationModel] = try hostsDirectory.retrieve(
          objectType: HostConfigurationModel.self, at: "hostsDirectory"
        ).compactMap { $0 }
        let hostParameters = hostConfigurations.first?.configuration.first(where: {
          $0.key == selectedAccount.address.eip55Description
        })?.value
  ...
  public func fetchHostsConfiguration(with host: String) -> HostConfigurationResolve? {
    do {
      if let selectedAccount: EthereumWallet = try ethereumAccount.fetchSelectedWallet() {
        ...
        let hostParameters = hostConfigurations.first?.configuration.first(where: {
          $0.key == selectedAccount.address.eip55Description
        })?.value.first(where: { $0.host == host })
        return HostConfigurationResolve(
          address: selectedAccount.address.eip55Description, chainId: hostParameters?.chainId,
          name: selectedAccount.name.addingPercentEncoding(withAllowedCharacters: .alphanumerics)
            ?? "")
      }
    } catch {
      if let selectedAccount: EthereumWallet = try? ethereumAccount.fetchSelectedWallet() {
  ...
  public func update(configuration: HostConfigurationRequest) throws {
    do {
      ...
      let config = HostConfigurationModel.HostConfigurationParameters(
        host: configuration.host, chainId: configuration.chainId ?? "",
        option: configuration.option, favicon: configuration.favicon)
      ...
      if let currentHosts = fetchHosts() {
        ...
        let hostConfiguration = HostConfigurationModel(configuration: [
          account.address.eip55Description: [config]
        ])
  ...
  public func delete(configuration: DeleteHostConfigurationRequest) throws {
    do {
      ...
      if let currentHosts = fetchHosts() {
        ...
        let hostConfiguration = HostConfigurationModel(configuration: [
          account.address.eip55Description: []
        ])
        try hostsDirectory.write(hostConfiguration, at: "hostsDirectory")
      }
    } catch {
  ...
  private func fetchHostsParameters(with address: String) -> [HostConfigurationModel
    .HostConfigurationParameters]?
  {
    do {
      let hostConfigurations: [HostConfigurationModel] = try hostsDirectory.retrieve(
        objectType: HostConfigurationModel.self, at: "hostsDirectory"
      ...
      if let host = hostConfigurations.first,
        let parameters = host.configuration.first(where: { $0.key == address })
      {
