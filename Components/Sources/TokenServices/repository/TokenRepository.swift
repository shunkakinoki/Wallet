...
public protocol TokenRepository {
  ...
  func get(with token: String) -> AnyPublisher<[Token], Error>
...
public class TokenRepositoryImp: TokenRepository {
  ...
  private init(
    dataSource: TokenDataSource, session: SessionRepository,
    tokens: ArrayCache<Token>
  ) {
  ...
  public func get(with token: String) -> AnyPublisher<[Token], Error> {
    dataSource.fetch(from: token).eraseToAnyPublisher()
  }
