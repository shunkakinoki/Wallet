...
public final class EthereumAccount: Identifiable {
  ...
  public init(
    walletDirectory: WriterDirectory = DirectoryWalletWriterDirectory(fileSubfolder: "keystore/"),
...
extension EthereumAccount {
  ...
  private func retrieveSelectedEthereumWallet() throws -> EthereumWallet? {
    let selectedWallets: [String?] = try selectedDirectory.retrieve(
      objectType: String.self, at: "selectedWallet")
    guard let selectedWallet = selectedWallets.first as? String else {
      throw Error.fetchingSelectedWallet
    }
    let keystores: [KeyStore?] = try walletDirectory.retrieve(
      objectType: KeyStore.self, at: "selectedWallet")
    return
      try keystores
